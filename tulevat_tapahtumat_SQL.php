...
$kyselysql = "SELECT tapahtuma_nimi, tapahtuma_alku_paiva, tapahtuma_alku_aika, tapahtuma_paikka
FROM tapahtuma
WHERE DATE(tapahtuma_alku_paiva) >= CURDATE()
ORDER BY tapahtuma_alku_paiva ASC;";
...
// $paivamaara = "2023-09-30"; Tietokannasta haettu päivämäärä
  ...
  while ($rivit = mysqli_fetch_assoc($tulos)) {
      ...
      $suomalainen_paivamaara = strftime("%d.%m.%Y", strtotime($rivit['tapahtuma_alku_paiva']));

      echo "<td>" . $suomalainen_paivamaara . "</td>";
